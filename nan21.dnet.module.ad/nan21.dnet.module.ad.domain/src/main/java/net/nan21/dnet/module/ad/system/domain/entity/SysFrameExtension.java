...
@NamedQueries({
	@NamedQuery(
		name=SysFrameExtension.NQ_FIND_BY_ID,
		query="SELECT e FROM SysFrameExtension e WHERE e.clientId = :pClientId and e.id = :pId ",
		hints=@QueryHint(name=QueryHints.BIND_PARAMETERS, value=HintValues.TRUE)
	)
	,@NamedQuery(
		name=SysFrameExtension.NQ_FIND_BY_IDS,
		query="SELECT e FROM SysFrameExtension e WHERE e.clientId = :pClientId and e.id in :pIds",
		hints=@QueryHint(name=QueryHints.BIND_PARAMETERS, value=HintValues.TRUE)
	)
	,@NamedQuery(
		name=SysFrameExtension.NQ_FIND_BY_NAME,
		query="SELECT e FROM SysFrameExtension e WHERE e.clientId = :pClientId and e.frameFQN = :pFrameFQN and e.fileLocation = :pFileLocation",
		hints=@QueryHint(name=QueryHints.BIND_PARAMETERS, value=HintValues.TRUE)
	)
})
@Entity
@Table(
	name=SysFrameExtension.TABLE_NAME
	,uniqueConstraints={
		@UniqueConstraint( 
			name=SysFrameExtension.TABLE_NAME+"_UK1"
			,columnNames={"CLIENTID","FRAMEFQN","FILELOCATION"}
		)
	}
)
