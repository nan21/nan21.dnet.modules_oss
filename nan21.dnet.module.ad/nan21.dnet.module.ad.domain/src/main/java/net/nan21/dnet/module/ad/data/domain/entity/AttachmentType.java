...
@NamedQueries({
	@NamedQuery(
		name=AttachmentType.NQ_FIND_BY_ID,
		query="SELECT e FROM AttachmentType e WHERE e.clientId = :pClientId and e.id = :pId ",
		hints=@QueryHint(name=QueryHints.BIND_PARAMETERS, value=HintValues.TRUE)
	)
	,@NamedQuery(
		name=AttachmentType.NQ_FIND_BY_IDS,
		query="SELECT e FROM AttachmentType e WHERE e.clientId = :pClientId and e.id in :pIds",
		hints=@QueryHint(name=QueryHints.BIND_PARAMETERS, value=HintValues.TRUE)
	)
	,@NamedQuery(
		name=AttachmentType.NQ_FIND_BY_NAME,
		query="SELECT e FROM AttachmentType e WHERE e.clientId = :pClientId and e.name = :pName",
		hints=@QueryHint(name=QueryHints.BIND_PARAMETERS, value=HintValues.TRUE)
	)
})
@Entity
@Table(
	name=AttachmentType.TABLE_NAME
	,uniqueConstraints={
		@UniqueConstraint( 
			name=AttachmentType.TABLE_NAME+"_UK1"
