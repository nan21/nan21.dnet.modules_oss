...
@NamedQueries({
	@NamedQuery(
		name=Client.NQ_FIND_BY_ID,
		query="SELECT e FROM Client e WHERE  e.id = :pId ",
		hints=@QueryHint(name=QueryHints.BIND_PARAMETERS, value=HintValues.TRUE)
	)
	,@NamedQuery(
		name=Client.NQ_FIND_BY_IDS,
		query="SELECT e FROM Client e WHERE  e.id in :pIds",
		hints=@QueryHint(name=QueryHints.BIND_PARAMETERS, value=HintValues.TRUE)
	)
})
@Entity
...
public class Client  implements IModelWithId {
	...
	public void aboutToInsert(DescriptorEvent event) {
		...
		event.updateAttributeWithObject("createdBy", Session.user.get().getUsername() );
		event.updateAttributeWithObject("modifiedBy", Session.user.get().getUsername() );
		if (this.uuid == null || this.uuid.equals("") ) {	
			event.updateAttributeWithObject("uuid", UUID.randomUUID().toString().toUpperCase());
		}
