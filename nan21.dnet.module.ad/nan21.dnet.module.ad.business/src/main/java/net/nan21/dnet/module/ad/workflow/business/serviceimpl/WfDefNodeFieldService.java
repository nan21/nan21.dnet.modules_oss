...
public class WfDefNodeFieldService extends AbstractEntityService<WfDefNodeField>
		implements IWfDefNodeFieldService {
	...
	public List<WfDefNodeField> findByNodeId(Long nodeId) {
		return (List<WfDefNodeField>) this.em
			.createQuery("select e from WfDefNodeField e where e.clientId = :pClientId and e.node.id = :pNodeId", WfDefNodeField.class)
			.setParameter("pClientId", Session.user.get().getClientId())
