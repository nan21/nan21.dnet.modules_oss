...
public class SysDataSourceService extends AbstractEntityService<SysDataSource>
	...
	public List<SysDataSource> findByFieldsId(Long fieldsId) {
		return (List<SysDataSource>) this.em
			.createQuery("select distinct e from SysDataSource e, IN (e.fields) c where e.clientId = :pClientId and c.id = :pFieldsId", SysDataSource.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<SysDataSource> findByServiceMethodsId(Long serviceMethodsId) {
		return (List<SysDataSource>) this.em
			.createQuery("select distinct e from SysDataSource e, IN (e.serviceMethods) c where e.clientId = :pClientId and c.id = :pServiceMethodsId", SysDataSource.class)
			.setParameter("pClientId", Session.user.get().getClientId())
