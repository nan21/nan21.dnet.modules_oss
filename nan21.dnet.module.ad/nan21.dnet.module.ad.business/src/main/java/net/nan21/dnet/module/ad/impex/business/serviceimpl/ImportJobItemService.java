...
public class ImportJobItemService extends AbstractEntityService<ImportJobItem>
	...
	public ImportJobItem findByJob_map(ImportJob job,ImportMap map) {		 
		return (ImportJobItem) this.em
			...
			.setParameter("pJob", job)
			.setParameter("pMap", map)
			.getSingleResult(); 
	}
	...
	public List<ImportJobItem> findByJobId(Long jobId) {
		return (List<ImportJobItem>) this.em
			.createQuery("select e from ImportJobItem e where e.clientId = :pClientId and e.job.id = :pJobId", ImportJobItem.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pJobId", jobId)			 	
			.getResultList(); 
	}
	...
	public List<ImportJobItem> findByMapId(Long mapId) {
		return (List<ImportJobItem>) this.em
			.createQuery("select e from ImportJobItem e where e.clientId = :pClientId and e.map.id = :pMapId", ImportJobItem.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pMapId", mapId)			 	
			.getResultList(); 
	}
