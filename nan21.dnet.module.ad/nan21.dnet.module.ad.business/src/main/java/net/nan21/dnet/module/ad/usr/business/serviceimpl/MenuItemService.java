...
public class MenuItemService extends AbstractEntityService<MenuItem>
	...
	public List<MenuItem> findByMenuItemId(Long menuItemId) {
		return (List<MenuItem>) this.em
			.createQuery("select e from MenuItem e where e.clientId = :pClientId and e.menuItem.id = :pMenuItemId", MenuItem.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pMenuItemId", menuItemId)			 	
			.getResultList(); 
	}
	...
	public List<MenuItem> findByMenuId(Long menuId) {
		return (List<MenuItem>) this.em
			.createQuery("select e from MenuItem e where e.clientId = :pClientId and e.menu.id = :pMenuId", MenuItem.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pMenuId", menuId)			 	
			.getResultList(); 
	}
	...
	public List<MenuItem> findByRolesId(Long rolesId) {
		return (List<MenuItem>) this.em
			.createQuery("select distinct e from MenuItem e, IN (e.roles) c where e.clientId = :pClientId and c.id = :pRolesId", MenuItem.class)
			.setParameter("pClientId", Session.user.get().getClientId())
