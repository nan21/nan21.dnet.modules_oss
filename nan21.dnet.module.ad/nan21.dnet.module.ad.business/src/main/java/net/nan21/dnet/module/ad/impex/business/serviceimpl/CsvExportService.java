...
public class CsvExportService extends AbstractEntityService<CsvExport>
	...
	public List<CsvExport> findByFieldsId(Long fieldsId) {
		return (List<CsvExport>) this.em
			.createQuery("select distinct e from CsvExport e, IN (e.fields) c where e.clientId = :pClientId and c.id = :pFieldsId", CsvExport.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pFieldsId", fieldsId)			 	
			.getResultList(); 
	}
	...
	public List<CsvExport> findBySortsId(Long sortsId) {
		return (List<CsvExport>) this.em
			.createQuery("select distinct e from CsvExport e, IN (e.sorts) c where e.clientId = :pClientId and c.id = :pSortsId", CsvExport.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pSortsId", sortsId)			 	
			.getResultList(); 
	}
}
