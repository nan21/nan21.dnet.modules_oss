...
public class WfDefProcessService extends AbstractEntityService<WfDefProcess>
	...
	public List<WfDefProcess> findByNodesId(Long nodesId) {
		return (List<WfDefProcess>) this.em
			.createQuery("select distinct e from WfDefProcess e, IN (e.nodes) c where e.clientId = :pClientId and c.id = :pNodesId", WfDefProcess.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pNodesId", nodesId)			 	
			.getResultList(); 
	}
}
