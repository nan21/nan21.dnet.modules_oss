...
public class WfDefNodeService extends AbstractEntityService<WfDefNode>
	...
	public List<WfDefNode> findByProcessId(Long processId) {
		return (List<WfDefNode>) this.em
			.createQuery("select e from WfDefNode e where e.clientId = :pClientId and e.process.id = :pProcessId", WfDefNode.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<WfDefNode> findByFieldsId(Long fieldsId) {
		return (List<WfDefNode>) this.em
			.createQuery("select distinct e from WfDefNode e, IN (e.fields) c where e.clientId = :pClientId and c.id = :pFieldsId", WfDefNode.class)
			.setParameter("pClientId", Session.user.get().getClientId())
