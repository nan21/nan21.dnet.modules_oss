...
public class RoleService extends AbstractEntityService<Role>
	...
	public List<Role> findByUsersId(Long usersId) {
		return (List<Role>) this.em
			.createQuery("select distinct e from Role e, IN (e.users) c where e.clientId = :pClientId and c.id = :pUsersId", Role.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<Role> findByAccessControlsId(Long accessControlsId) {
		return (List<Role>) this.em
			.createQuery("select distinct e from Role e, IN (e.accessControls) c where e.clientId = :pClientId and c.id = :pAccessControlsId", Role.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<Role> findByMenusId(Long menusId) {
		return (List<Role>) this.em
			.createQuery("select distinct e from Role e, IN (e.menus) c where e.clientId = :pClientId and c.id = :pMenusId", Role.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pMenusId", menusId)			 	
			.getResultList(); 
	}
	...
	public List<Role> findByMenuItemsId(Long menuItemsId) {
		return (List<Role>) this.em
			.createQuery("select distinct e from Role e, IN (e.menuItems) c where e.clientId = :pClientId and c.id = :pMenuItemsId", Role.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pMenuItemsId", menuItemsId)			 	
			.getResultList(); 
	}
}
