...
public class UserService extends AbstractEntityService<User>
	...
	public List<User> findByDateFormatId(Long dateFormatId) {
		return (List<User>) this.em
			.createQuery("select e from User e where e.clientId = :pClientId and e.dateFormat.id = :pDateFormatId", User.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pDateFormatId", dateFormatId)			 	
			.getResultList(); 
	}
	...
	public List<User> findByAccountTypeId(Long accountTypeId) {
		return (List<User>) this.em
			.createQuery("select e from User e where e.clientId = :pClientId and e.accountType.id = :pAccountTypeId", User.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pAccountTypeId", accountTypeId)			 	
			.getResultList(); 
	}
	...
	public List<User> findByRolesId(Long rolesId) {
		return (List<User>) this.em
			.createQuery("select distinct e from User e, IN (e.roles) c where e.clientId = :pClientId and c.id = :pRolesId", User.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pRolesId", rolesId)			 	
			.getResultList(); 
	}
	...
	public List<User> findByGroupsId(Long groupsId) {
		return (List<User>) this.em
			.createQuery("select distinct e from User e, IN (e.groups) c where e.clientId = :pClientId and c.id = :pGroupsId", User.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pGroupsId", groupsId)			 	
			.getResultList(); 
	}
	
	public void doChangePassword(Long userId, String newPassword) throws Exception {
