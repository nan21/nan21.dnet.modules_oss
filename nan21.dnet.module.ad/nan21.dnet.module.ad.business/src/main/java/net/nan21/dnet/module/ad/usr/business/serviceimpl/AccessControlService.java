...
public class AccessControlService extends AbstractEntityService<AccessControl>
	...
	public List<AccessControl> findByDsRulesId(Long dsRulesId) {
		return (List<AccessControl>) this.em
			.createQuery("select distinct e from AccessControl e, IN (e.dsRules) c where e.clientId = :pClientId and c.id = :pDsRulesId", AccessControl.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pDsRulesId", dsRulesId)			 	
			.getResultList(); 
	}
	...
	public List<AccessControl> findByRolesId(Long rolesId) {
		return (List<AccessControl>) this.em
			.createQuery("select distinct e from AccessControl e, IN (e.roles) c where e.clientId = :pClientId and c.id = :pRolesId", AccessControl.class)
			.setParameter("pClientId", Session.user.get().getClientId())
