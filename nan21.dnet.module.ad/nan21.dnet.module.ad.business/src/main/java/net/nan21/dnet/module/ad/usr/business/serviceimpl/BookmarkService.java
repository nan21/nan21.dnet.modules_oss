...
public class BookmarkService extends AbstractEntityService<Bookmark>
	...
	public List<Bookmark> findByParentId(Long parentId) {
		return (List<Bookmark>) this.em
			.createQuery("select e from Bookmark e where e.clientId = :pClientId and e.parent.id = :pParentId", Bookmark.class)
			.setParameter("pClientId", Session.user.get().getClientId())
