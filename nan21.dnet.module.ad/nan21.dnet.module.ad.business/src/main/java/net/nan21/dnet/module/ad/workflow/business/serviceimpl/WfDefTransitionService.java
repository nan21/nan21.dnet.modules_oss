...
public class WfDefTransitionService extends AbstractEntityService<WfDefTransition>
		implements IWfDefTransitionService {
	...
	public List<WfDefTransition> findByProcessId(Long processId) {
		return (List<WfDefTransition>) this.em
			.createQuery("select e from WfDefTransition e where e.clientId = :pClientId and e.process.id = :pProcessId", WfDefTransition.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pProcessId", processId)			 	
			.getResultList(); 
	}
	...
	public List<WfDefTransition> findBySourceId(Long sourceId) {
		return (List<WfDefTransition>) this.em
			.createQuery("select e from WfDefTransition e where e.clientId = :pClientId and e.source.id = :pSourceId", WfDefTransition.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<WfDefTransition> findByTargetId(Long targetId) {
		return (List<WfDefTransition>) this.em
			.createQuery("select e from WfDefTransition e where e.clientId = :pClientId and e.target.id = :pTargetId", WfDefTransition.class)
			.setParameter("pClientId", Session.user.get().getClientId())
