...
public class Setup_AD extends AbstractPresenterSetupParticipant implements
	...
	protected void onExecute() throws Exception {
...

		{
			...
			ImportJob ij = (ImportJob) importJobService
					.getEntityManager()
					.createQuery(
							"select e from " + ImportJob.class.getSimpleName()
									+ " e where e.name = :name")
					.setParameter("name", "Initial setup data").getResultList()
					.get(0);
		...
		if (b) {
			...
			ImportJob ij = (ImportJob) importJobService
					...
					.createQuery(
							"select e from " + ImportJob.class.getSimpleName()
									+ " e where e.name = :name")
					.setParameter("name", "Initial demo data").getResultList()
					.get(0);
	...
	private void createTasks() {
		...
		try {
			...
			List<ImportJob> ijlist = (List<ImportJob>) this
					.findEntityService(ImportJob.class)
					.getEntityManager()
					.createQuery(
							"select e from " + ImportJob.class.getSimpleName()
									+ " e", ImportJob.class).getResultList();
