...
public class MenuItemDsConv extends AbstractDsConverter<MenuItemDs, MenuItem> 
    ...
    @Override
    protected void modelToEntityReferences(MenuItemDs ds, MenuItem e, boolean isInsert) throws Exception {
    	if( ds.getMenuItemId() != null  ) {
    		if (e.getMenuItem() == null || !e.getMenuItem().getId().equals(ds.getMenuItemId()) ) {
    			e.setMenuItem( (MenuItem) this.em.find(MenuItem.class, ds.getMenuItemId() ) );
    		}
    	}
    	else {
    	...
    	if( ds.getMenuId() != null  ) {
    		if (e.getMenu() == null || !e.getMenu().getId().equals(ds.getMenuId()) ) {
    ...
    protected void lookup_menuItem_MenuItem(MenuItemDs ds, MenuItem e ) throws Exception {
    	if (ds.getMenuItem() != null && !ds.getMenuItem().equals("") ) {
    		MenuItem x = null;
    		try { 
    			x = ((IMenuItemService)findEntityService(MenuItem.class)).findByName( ds.getMenuItem() );
    		} catch(javax.persistence.NoResultException exception) {
    			throw new Exception("Invalid value provided to find `MenuItem` reference: `menuItem` = " + ds.getMenuItem() + "" );
    		}
    		e.setMenuItem(x); 
    	} else {
    		e.setMenuItem(null);
    ...
    protected void lookup_menu_Menu(MenuItemDs ds, MenuItem e ) throws Exception {
    	if (ds.getMenu() != null && !ds.getMenu().equals("") ) {
    		Menu x = null;
    		try { 
    			x = ((IMenuService)findEntityService(Menu.class)).findByName( ds.getMenu() );
    		} catch(javax.persistence.NoResultException exception) {
    			throw new Exception("Invalid value provided to find `Menu` reference: `menu` = " + ds.getMenu() + "" );
    		}
    		e.setMenu(x); 
    	} else {
    		e.setMenu(null);
