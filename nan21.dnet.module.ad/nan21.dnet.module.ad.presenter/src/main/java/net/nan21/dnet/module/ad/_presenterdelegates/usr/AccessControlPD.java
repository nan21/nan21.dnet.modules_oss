...
public class AccessControlPD extends AbstractPresenterBaseService {
	...
	public void copyRulesFromAccessControl(AccessControlDs ds,
			AccessControlDsParam params) throws Exception {
		...
		IDsAccessControlService dsACService = (IDsAccessControlService) this
				.findEntityService(DsAccessControl.class);
		...
		if (!params.getSkipDs()) {
			...
			List<DsAccessControl> sourceDsRulesList = dsACService
		...
		if (!params.getSkipAsgn()) {
			IAsgnAccessControlService asgnACService = (IAsgnAccessControlService) this
					.findEntityService(AsgnAccessControl.class);

			List<AsgnAccessControl> sourceAsgnRulesList = asgnACService
					.findByAccessControlId(params.getCopyFromId());
