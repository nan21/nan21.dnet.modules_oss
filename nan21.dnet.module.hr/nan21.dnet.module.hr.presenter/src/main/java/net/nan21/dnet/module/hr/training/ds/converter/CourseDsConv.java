...
public class CourseDsConv extends AbstractDsConverter<CourseDs, Course> 
    ...
    @Override
    protected void modelToEntityReferences(CourseDs ds, Course e, boolean isInsert) throws Exception {
    	if( ds.getTypeId() != null  ) {
    		if (e.getType() == null || !e.getType().getId().equals(ds.getTypeId()) ) {
    			e.setType( (CourseType) this.em.find(CourseType.class, ds.getTypeId() ) );
    		}
    	}
    	else {
    ...
    protected void lookup_type_CourseType(CourseDs ds, Course e ) throws Exception {
    	if (ds.getType() != null && !ds.getType().equals("") ) {
    		CourseType x = null;
    		try { 
    			x = ((ICourseTypeService)findEntityService(CourseType.class)).findByName( ds.getType() );
    		} catch(javax.persistence.NoResultException exception) {
    			throw new Exception("Invalid value provided to find `CourseType` reference: `type` = " + ds.getType() + "" );
    		}
    		e.setType(x); 
    	} else {
    		e.setType(null);
