...
public class JobGradeService extends AbstractEntityService<JobGrade>
	...
	public List<JobGrade> findByJobId(Long jobId) {
		return (List<JobGrade>) this.em
			.createQuery("select e from JobGrade e where e.clientId = :pClientId and e.job.id = :pJobId", JobGrade.class)
	...
	public List<JobGrade> findByGradeId(Long gradeId) {
		return (List<JobGrade>) this.em
			.createQuery("select e from JobGrade e where e.clientId = :pClientId and e.grade.id = :pGradeId", JobGrade.class)
			.setParameter("pClientId", Session.user.get().getClientId())
