...
public class PayScaleRateService extends AbstractEntityService<PayScaleRate>
	...
	public List<PayScaleRate> findByPayScaleId(Long payScaleId) {
		return (List<PayScaleRate>) this.em
			.createQuery("select e from PayScaleRate e where e.clientId = :pClientId and e.payScale.id = :pPayScaleId", PayScaleRate.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<PayScaleRate> findByCurrencyId(Long currencyId) {
		return (List<PayScaleRate>) this.em
			.createQuery("select e from PayScaleRate e where e.clientId = :pClientId and e.currency.id = :pCurrencyId", PayScaleRate.class)
			.setParameter("pClientId", Session.user.get().getClientId())
