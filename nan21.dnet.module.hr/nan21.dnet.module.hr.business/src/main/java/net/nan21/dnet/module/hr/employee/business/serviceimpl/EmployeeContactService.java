...
public class EmployeeContactService extends AbstractEntityService<EmployeeContact>
		implements IEmployeeContactService {
	...
	public List<EmployeeContact> findByEmployeeId(Long employeeId) {
		return (List<EmployeeContact>) this.em
			.createQuery("select e from EmployeeContact e where e.clientId = :pClientId and e.employee.id = :pEmployeeId", EmployeeContact.class)
	...
	public List<EmployeeContact> findByTypeId(Long typeId) {
		return (List<EmployeeContact>) this.em
			.createQuery("select e from EmployeeContact e where e.clientId = :pClientId and e.type.id = :pTypeId", EmployeeContact.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pTypeId", typeId)			 	
			.getResultList(); 
	}
	
	public List<EmployeeContact> findByRelationship(EmployeeContactRelationship relationship) {
		return this.findByRelationshipId(relationship.getId()); 
	}
	
	public List<EmployeeContact> findByRelationshipId(Long relationshipId) {
		return (List<EmployeeContact>) this.em
			.createQuery("select e from EmployeeContact e where e.clientId = :pClientId and e.relationship.id = :pRelationshipId", EmployeeContact.class)
			.setParameter("pClientId", Session.user.get().getClientId())
