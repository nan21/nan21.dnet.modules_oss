...
public class SkillService extends AbstractEntityService<Skill>
	...
	public List<Skill> findByTypeId(Long typeId) {
		return (List<Skill>) this.em
			.createQuery("select e from Skill e where e.clientId = :pClientId and e.type.id = :pTypeId", Skill.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<Skill> findByRatingScaleId(Long ratingScaleId) {
		return (List<Skill>) this.em
			.createQuery("select e from Skill e where e.clientId = :pClientId and e.ratingScale.id = :pRatingScaleId", Skill.class)
			.setParameter("pClientId", Session.user.get().getClientId())
