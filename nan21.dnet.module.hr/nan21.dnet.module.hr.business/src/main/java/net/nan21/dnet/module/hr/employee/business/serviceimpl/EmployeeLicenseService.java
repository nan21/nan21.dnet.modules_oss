...
public class EmployeeLicenseService extends AbstractEntityService<EmployeeLicense>
		implements IEmployeeLicenseService {
	...
	public List<EmployeeLicense> findByEmployeeId(Long employeeId) {
		return (List<EmployeeLicense>) this.em
			.createQuery("select e from EmployeeLicense e where e.clientId = :pClientId and e.employee.id = :pEmployeeId", EmployeeLicense.class)
	...
	public List<EmployeeLicense> findByLicenseTypeId(Long licenseTypeId) {
		return (List<EmployeeLicense>) this.em
			.createQuery("select e from EmployeeLicense e where e.clientId = :pClientId and e.licenseType.id = :pLicenseTypeId", EmployeeLicense.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pLicenseTypeId", licenseTypeId)			 	
			.getResultList(); 
	}
}
