...
public class SalesOrderTaxService extends AbstractEntityService<SalesOrderTax>
	...
	public List<SalesOrderTax> findBySalesOrderId(Long salesOrderId) {
		return (List<SalesOrderTax>) this.em
			.createQuery("select e from SalesOrderTax e where e.clientId = :pClientId and e.salesOrder.id = :pSalesOrderId", SalesOrderTax.class)
	...
	public List<SalesOrderTax> findByTaxId(Long taxId) {
		return (List<SalesOrderTax>) this.em
			.createQuery("select e from SalesOrderTax e where e.clientId = :pClientId and e.tax.id = :pTaxId", SalesOrderTax.class)
			.setParameter("pClientId", Session.user.get().getClientId())
