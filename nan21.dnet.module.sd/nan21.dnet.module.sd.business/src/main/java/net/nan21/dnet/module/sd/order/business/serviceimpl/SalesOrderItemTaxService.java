...
public class SalesOrderItemTaxService extends AbstractEntityService<SalesOrderItemTax>
		implements ISalesOrderItemTaxService {
	...
	public List<SalesOrderItemTax> findBySalesOrderItemId(Long salesOrderItemId) {
		return (List<SalesOrderItemTax>) this.em
			.createQuery("select e from SalesOrderItemTax e where e.clientId = :pClientId and e.salesOrderItem.id = :pSalesOrderItemId", SalesOrderItemTax.class)
	...
	public List<SalesOrderItemTax> findByTaxId(Long taxId) {
		return (List<SalesOrderItemTax>) this.em
			.createQuery("select e from SalesOrderItemTax e where e.clientId = :pClientId and e.tax.id = :pTaxId", SalesOrderItemTax.class)
			.setParameter("pClientId", Session.user.get().getClientId())
