...
public class ElementInputService extends AbstractEntityService<ElementInput>
	...
	public List<ElementInput> findByElementId(Long elementId) {
		return (List<ElementInput>) this.em
			.createQuery("select e from ElementInput e where e.clientId = :pClientId and e.element.id = :pElementId", ElementInput.class)
	...
	public List<ElementInput> findByCrossReferenceId(Long crossReferenceId) {
		return (List<ElementInput>) this.em
			.createQuery("select e from ElementInput e where e.clientId = :pClientId and e.crossReference.id = :pCrossReferenceId", ElementInput.class)
			.setParameter("pClientId", Session.user.get().getClientId())
