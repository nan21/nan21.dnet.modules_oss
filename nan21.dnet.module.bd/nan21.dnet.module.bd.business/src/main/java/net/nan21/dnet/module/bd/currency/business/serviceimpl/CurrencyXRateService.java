...
public class CurrencyXRateService extends AbstractEntityService<CurrencyXRate>
	...
	public CurrencyXRate findByValid(CurrencyXRateProvider provider,Currency source,Currency target,Date validAt) {		 
	...
	public CurrencyXRate findByValid( Long providerId, Long sourceId, Long targetId,Date validAt) {
		return (CurrencyXRate) this.em
	...
	public List<CurrencyXRate> findByProviderId(Long providerId) {
		return (List<CurrencyXRate>) this.em
			.createQuery("select e from CurrencyXRate e where e.clientId = :pClientId and e.provider.id = :pProviderId", CurrencyXRate.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pProviderId", providerId)			 	
			.getResultList(); 
	}
	...
	public List<CurrencyXRate> findBySourceId(Long sourceId) {
		return (List<CurrencyXRate>) this.em
			.createQuery("select e from CurrencyXRate e where e.clientId = :pClientId and e.source.id = :pSourceId", CurrencyXRate.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pSourceId", sourceId)			 	
			.getResultList(); 
	}
	...
	public List<CurrencyXRate> findByTargetId(Long targetId) {
		return (List<CurrencyXRate>) this.em
			.createQuery("select e from CurrencyXRate e where e.clientId = :pClientId and e.target.id = :pTargetId", CurrencyXRate.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pTargetId", targetId)			 	
			.getResultList(); 
	}
