...
public class ClassificationItemService extends AbstractEntityService<ClassificationItem>
		implements IClassificationItemService {
	...
	public ClassificationItem findBySyscode( Long classSystemId,String code) {
		return (ClassificationItem) this.em
			.createNamedQuery(ClassificationItem.NQ_FIND_BY_SYSCODE_PRIMITIVE)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public ClassificationItem findBySysname( Long classSystemId,String name) {
		return (ClassificationItem) this.em
			.createNamedQuery(ClassificationItem.NQ_FIND_BY_SYSNAME_PRIMITIVE)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pClassSystemId", classSystemId)
			.setParameter("pName", name)
			.getSingleResult(); 
	...
	public List<ClassificationItem> findByClassSystemId(Long classSystemId) {
		return (List<ClassificationItem>) this.em
			.createQuery("select e from ClassificationItem e where e.clientId = :pClientId and e.classSystem.id = :pClassSystemId", ClassificationItem.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pClassSystemId", classSystemId)			 	
			.getResultList(); 
	}
