...
public class UomConversionService extends AbstractEntityService<UomConversion>
	...
	public List<UomConversion> findBySourceId(Long sourceId) {
		return (List<UomConversion>) this.em
			.createQuery("select e from UomConversion e where e.clientId = :pClientId and e.source.id = :pSourceId", UomConversion.class)
	...
	public List<UomConversion> findByTargetId(Long targetId) {
		return (List<UomConversion>) this.em
			.createQuery("select e from UomConversion e where e.clientId = :pClientId and e.target.id = :pTargetId", UomConversion.class)
			.setParameter("pClientId", Session.user.get().getClientId())
