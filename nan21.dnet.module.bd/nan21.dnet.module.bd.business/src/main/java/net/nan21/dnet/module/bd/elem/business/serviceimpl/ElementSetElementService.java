...
public class ElementSetElementService extends AbstractEntityService<ElementSetElement>
		implements IElementSetElementService {
	...
	public ElementSetElement findBySet_elem(ElementSet elementSet,Element element) {		 
		return (ElementSetElement) this.em
			...
			.setParameter("pElement", element)
			.getSingleResult(); 
	}
	
	public ElementSetElement findBySet_elem( Long elementSetId, Long elementId) {
		return (ElementSetElement) this.em
			.createNamedQuery(ElementSetElement.NQ_FIND_BY_SET_ELEM_PRIMITIVE)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<ElementSetElement> findByElementSetId(Long elementSetId) {
		return (List<ElementSetElement>) this.em
			.createQuery("select e from ElementSetElement e where e.clientId = :pClientId and e.elementSet.id = :pElementSetId", ElementSetElement.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pElementSetId", elementSetId)			 	
			.getResultList(); 
	}
	...
	public List<ElementSetElement> findByElementId(Long elementId) {
		return (List<ElementSetElement>) this.em
			.createQuery("select e from ElementSetElement e where e.clientId = :pClientId and e.element.id = :pElementId", ElementSetElement.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pElementId", elementId)			 	
			.getResultList(); 
	}
