...
@NamedQueries({
	@NamedQuery(
		name=OrganizationType.NQ_FIND_BY_ID,
		query="SELECT e FROM OrganizationType e WHERE e.clientId = :pClientId and e.id = :pId ",
		hints=@QueryHint(name=QueryHints.BIND_PARAMETERS, value=HintValues.TRUE)
	)
	,@NamedQuery(
		name=OrganizationType.NQ_FIND_BY_IDS,
		query="SELECT e FROM OrganizationType e WHERE e.clientId = :pClientId and e.id in :pIds",
		hints=@QueryHint(name=QueryHints.BIND_PARAMETERS, value=HintValues.TRUE)
	)
	,@NamedQuery(
		name=OrganizationType.NQ_FIND_BY_NAME,
		query="SELECT e FROM OrganizationType e WHERE e.clientId = :pClientId and e.name = :pName",
		hints=@QueryHint(name=QueryHints.BIND_PARAMETERS, value=HintValues.TRUE)
	)
})
@Entity
@Table(
	name=OrganizationType.TABLE_NAME
	,uniqueConstraints={
		@UniqueConstraint( 
			name=OrganizationType.TABLE_NAME+"_UK1"
