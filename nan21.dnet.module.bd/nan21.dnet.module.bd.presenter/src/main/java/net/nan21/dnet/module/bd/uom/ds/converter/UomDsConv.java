...
public class UomDsConv extends AbstractDsConverter<UomDs, Uom> 
		implements IDsConverter<UomDs, Uom> {
    
    @Override
    protected void modelToEntityReferences(UomDs ds, Uom e, boolean isInsert) throws Exception {
    ...
    protected void lookup_type_UomType(UomDs ds, Uom e ) throws Exception {
    	if (ds.getType() != null && !ds.getType().equals("") ) {
    		UomType x = null;
    		try { 
    			x = ((IUomTypeService)findEntityService(UomType.class)).findByName( ds.getType() );
    		} catch(javax.persistence.NoResultException exception) {
    			throw new Exception("Invalid value provided to find `UomType` reference: `type` = " + ds.getType() + "" );
    		}
    		e.setType(x); 
    	} else {
    		e.setType(null);
