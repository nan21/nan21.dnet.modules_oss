...
public class ElementTypeDsConv extends AbstractDsConverter<ElementTypeDs, ElementType> 
		implements IDsConverter<ElementTypeDs, ElementType> {
    
    @Override
    protected void modelToEntityReferences(ElementTypeDs ds, ElementType e, boolean isInsert) throws Exception {
    	...
    	if( ds.getCategoryId() != null  ) {
    		if (e.getCategory() == null || !e.getCategory().getId().equals(ds.getCategoryId()) ) {
    			e.setCategory( (ElementCategory) this.em.find(ElementCategory.class, ds.getCategoryId() ) );
    		}
    	}
    	else {
    ...
    protected void lookup_engine_Engine(ElementTypeDs ds, ElementType e ) throws Exception {
    ...
    protected void lookup_category_ElementCategory(ElementTypeDs ds, ElementType e ) throws Exception {
    	if (ds.getEngineId() != null && !ds.getEngineId().equals("")  && ds.getCategory() != null && !ds.getCategory().equals("") ) {
    		ElementCategory x = null;
    		try { 
    			x = ((IElementCategoryService)findEntityService(ElementCategory.class)).findByEngine_name( ds.getEngineId(),  ds.getCategory() );
    		} catch(javax.persistence.NoResultException exception) {
    			throw new Exception("Invalid value provided to find `ElementCategory` reference: `engineId` = " + ds.getEngineId() + ", `category` = " + ds.getCategory() + "" );
    		}
    		e.setCategory(x); 
    	} else {
    		e.setCategory(null);
