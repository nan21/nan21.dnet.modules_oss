...
public class PurchaseOrderTaxService extends AbstractEntityService<PurchaseOrderTax>
		implements IPurchaseOrderTaxService {
	...
	public List<PurchaseOrderTax> findByPurchaseOrderId(Long purchaseOrderId) {
		return (List<PurchaseOrderTax>) this.em
			.createQuery("select e from PurchaseOrderTax e where e.clientId = :pClientId and e.purchaseOrder.id = :pPurchaseOrderId", PurchaseOrderTax.class)
	...
	public List<PurchaseOrderTax> findByTaxId(Long taxId) {
		return (List<PurchaseOrderTax>) this.em
			.createQuery("select e from PurchaseOrderTax e where e.clientId = :pClientId and e.tax.id = :pTaxId", PurchaseOrderTax.class)
			.setParameter("pClientId", Session.user.get().getClientId())
