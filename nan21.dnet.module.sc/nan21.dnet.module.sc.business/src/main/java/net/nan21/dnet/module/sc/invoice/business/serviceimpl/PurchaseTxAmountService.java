...
public class PurchaseTxAmountService extends AbstractEntityService<PurchaseTxAmount>
	...
	public List<PurchaseTxAmount> findByOrderId(Long orderId) {
		return (List<PurchaseTxAmount>) this.em
			.createQuery("select e from PurchaseTxAmount e where e.clientId = :pClientId and e.order.id = :pOrderId", PurchaseTxAmount.class)
	...
	public List<PurchaseTxAmount> findByInvoiceId(Long invoiceId) {
		return (List<PurchaseTxAmount>) this.em
			.createQuery("select e from PurchaseTxAmount e where e.clientId = :pClientId and e.invoice.id = :pInvoiceId", PurchaseTxAmount.class)
			.setParameter("pClientId", Session.user.get().getClientId())
