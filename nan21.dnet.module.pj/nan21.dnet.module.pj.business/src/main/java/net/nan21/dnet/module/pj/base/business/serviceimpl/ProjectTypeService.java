...
public class ProjectTypeService extends AbstractEntityService<ProjectType>
	...
	public List<ProjectType> findByCategoryId(Long categoryId) {
		return (List<ProjectType>) this.em
			.createQuery("select e from ProjectType e where e.clientId = :pClientId and e.category.id = :pCategoryId", ProjectType.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<ProjectType> findByProjectRolesId(Long projectRolesId) {
		return (List<ProjectType>) this.em
			.createQuery("select distinct e from ProjectType e, IN (e.projectRoles) c where e.clientId = :pClientId and c.id = :pProjectRolesId", ProjectType.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<ProjectType> findByItemTypesId(Long itemTypesId) {
		return (List<ProjectType>) this.em
			.createQuery("select distinct e from ProjectType e, IN (e.itemTypes) c where e.clientId = :pClientId and c.id = :pItemTypesId", ProjectType.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pItemTypesId", itemTypesId)			 	
			.getResultList(); 
	}
}
