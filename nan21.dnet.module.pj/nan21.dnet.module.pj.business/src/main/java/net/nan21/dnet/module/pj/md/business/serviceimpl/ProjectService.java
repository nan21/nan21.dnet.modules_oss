...
public class ProjectService extends AbstractEntityService<Project>
	...
	public List<Project> findByTypeId(Long typeId) {
		return (List<Project>) this.em
			.createQuery("select e from Project e where e.clientId = :pClientId and e.type.id = :pTypeId", Project.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<Project> findByProjectLeadId(Long projectLeadId) {
		return (List<Project>) this.em
			.createQuery("select e from Project e where e.clientId = :pClientId and e.projectLead.id = :pProjectLeadId", Project.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pProjectLeadId", projectLeadId)			 	
			.getResultList(); 
	}
}
