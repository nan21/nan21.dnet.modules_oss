...
public class PaymentItemService extends AbstractEntityService<PaymentItem>
	...
	public List<PaymentItem> findByPaymentId(Long paymentId) {
		return (List<PaymentItem>) this.em
			.createQuery("select e from PaymentItem e where e.clientId = :pClientId and e.payment.id = :pPaymentId", PaymentItem.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pPaymentId", paymentId)			 	
			.getResultList(); 
	}
	...
	public List<PaymentItem> findByAccItemId(Long accItemId) {
		return (List<PaymentItem>) this.em
			.createQuery("select e from PaymentItem e where e.clientId = :pClientId and e.accItem.id = :pAccItemId", PaymentItem.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<PaymentItem> findByProductId(Long productId) {
		return (List<PaymentItem>) this.em
			.createQuery("select e from PaymentItem e where e.clientId = :pClientId and e.product.id = :pProductId", PaymentItem.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<PaymentItem> findByUomId(Long uomId) {
		return (List<PaymentItem>) this.em
			.createQuery("select e from PaymentItem e where e.clientId = :pClientId and e.uom.id = :pUomId", PaymentItem.class)
			.setParameter("pClientId", Session.user.get().getClientId())
