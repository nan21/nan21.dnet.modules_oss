...
public class FiscalPeriodStatusService extends AbstractEntityService<FiscalPeriodStatus>
		implements IFiscalPeriodStatusService {
	...
	public FiscalPeriodStatus findByOrg_period(Organization org,FiscalPeriod period) {		 
		return (FiscalPeriodStatus) this.em
			...
			.setParameter("pPeriod", period)
			.getSingleResult(); 
	}
	
	public FiscalPeriodStatus findByOrg_period( Long orgId, Long periodId) {
		return (FiscalPeriodStatus) this.em
			.createNamedQuery(FiscalPeriodStatus.NQ_FIND_BY_ORG_PERIOD_PRIMITIVE)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<FiscalPeriodStatus> findByPeriodId(Long periodId) {
		return (List<FiscalPeriodStatus>) this.em
			.createQuery("select e from FiscalPeriodStatus e where e.clientId = :pClientId and e.period.id = :pPeriodId", FiscalPeriodStatus.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pPeriodId", periodId)			 	
			.getResultList(); 
	}
	...
	public List<FiscalPeriodStatus> findByOrgId(Long orgId) {
		return (List<FiscalPeriodStatus>) this.em
			.createQuery("select e from FiscalPeriodStatus e where e.clientId = :pClientId and e.org.id = :pOrgId", FiscalPeriodStatus.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pOrgId", orgId)			 	
			.getResultList(); 
	}
