...
public class AccDocLineService extends AbstractEntityService<AccDocLine>
	...
	public List<AccDocLine> findByAccDocId(Long accDocId) {
		return (List<AccDocLine>) this.em
			.createQuery("select e from AccDocLine e where e.clientId = :pClientId and e.accDoc.id = :pAccDocId", AccDocLine.class)
	...
	public List<AccDocLine> findByProductId(Long productId) {
		return (List<AccDocLine>) this.em
			.createQuery("select e from AccDocLine e where e.clientId = :pClientId and e.product.id = :pProductId", AccDocLine.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<AccDocLine> findByAssetId(Long assetId) {
		return (List<AccDocLine>) this.em
			.createQuery("select e from AccDocLine e where e.clientId = :pClientId and e.asset.id = :pAssetId", AccDocLine.class)
			.setParameter("pClientId", Session.user.get().getClientId())
