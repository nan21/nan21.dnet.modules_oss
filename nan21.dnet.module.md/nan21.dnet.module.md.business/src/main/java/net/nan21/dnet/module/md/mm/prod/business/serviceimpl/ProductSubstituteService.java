...
public class ProductSubstituteService extends AbstractEntityService<ProductSubstitute>
		implements IProductSubstituteService {
	...
	public List<ProductSubstitute> findByRefProductId(Long refProductId) {
		return (List<ProductSubstitute>) this.em
			.createQuery("select e from ProductSubstitute e where e.clientId = :pClientId and e.refProduct.id = :pRefProductId", ProductSubstitute.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pRefProductId", refProductId)			 	
			.getResultList(); 
	}
	...
	public List<ProductSubstitute> findByEquivalenceId(Long equivalenceId) {
		return (List<ProductSubstitute>) this.em
			.createQuery("select e from ProductSubstitute e where e.clientId = :pClientId and e.equivalence.id = :pEquivalenceId", ProductSubstitute.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pEquivalenceId", equivalenceId)			 	
			.getResultList(); 
	}
}
