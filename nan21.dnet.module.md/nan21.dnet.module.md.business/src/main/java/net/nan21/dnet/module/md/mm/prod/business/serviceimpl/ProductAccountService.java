...
public class ProductAccountService extends AbstractEntityService<ProductAccount>
		implements IProductAccountService {
	...
	public List<ProductAccount> findByOrgId(Long orgId) {
		return (List<ProductAccount>) this.em
			.createQuery("select e from ProductAccount e where e.clientId = :pClientId and e.org.id = :pOrgId", ProductAccount.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pOrgId", orgId)			 	
			.getResultList(); 
	}
	...
	public List<ProductAccount> findByGroupId(Long groupId) {
		return (List<ProductAccount>) this.em
			.createQuery("select e from ProductAccount e where e.clientId = :pClientId and e.group.id = :pGroupId", ProductAccount.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<ProductAccount> findByProductId(Long productId) {
		return (List<ProductAccount>) this.em
			.createQuery("select e from ProductAccount e where e.clientId = :pClientId and e.product.id = :pProductId", ProductAccount.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pProductId", productId)			 	
			.getResultList(); 
	}
