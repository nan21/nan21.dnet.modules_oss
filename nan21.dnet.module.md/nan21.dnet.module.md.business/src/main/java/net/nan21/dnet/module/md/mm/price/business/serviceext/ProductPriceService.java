...
public class ProductPriceService
	...
	public ProductPrice getDefaultSalesProductPrice(Long productId,
		...
		PriceList pl = ((IPriceListService) this
				.findEntityService(PriceList.class)).findDefaultPriceList(true);
	...
	public ProductPrice getDefaultPurchaseProductPrice(Long productId,
			Date eventDate) throws BusinessException {
		PriceList pl = ((IPriceListService) this
				.findEntityService(PriceList.class))
				.findDefaultPriceList(false);
		return getProductPrice(productId, pl.getId(), eventDate);
	}
	...
	public ProductPrice getProductPrice(Long productId, Long priceListId,
		...
		Assert.notNull(productId,
				"No product specified to lookup the price for.");
		...
		String eql = "select pp from "
				...
				+ " pp join fetch pp.priceListVersion  "
				+
		...
		List<ProductPrice> results = (List<ProductPrice>) this.em
				...
				.setParameter("pValidAt", validAt).getResultList();

		if (results.size() == 0) {
			throw new BusinessException("No price defined for this product! ");
		} else
