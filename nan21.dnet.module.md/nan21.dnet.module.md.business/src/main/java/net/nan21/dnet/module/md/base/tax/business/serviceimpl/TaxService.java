...
public class TaxService extends AbstractEntityService<Tax>
	...
	public List<Tax> findByCategoryId(Long categoryId) {
		return (List<Tax>) this.em
			.createQuery("select e from Tax e where e.clientId = :pClientId and e.category.id = :pCategoryId", Tax.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<Tax> findByParentTaxId(Long parentTaxId) {
		return (List<Tax>) this.em
			.createQuery("select e from Tax e where e.clientId = :pClientId and e.parentTax.id = :pParentTaxId", Tax.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pParentTaxId", parentTaxId)			 	
			.getResultList(); 
	}
	...
	public List<Tax> findByChildrenId(Long childrenId) {
		return (List<Tax>) this.em
			.createQuery("select distinct e from Tax e, IN (e.children) c where e.clientId = :pClientId and c.id = :pChildrenId", Tax.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pChildrenId", childrenId)			 	
			.getResultList(); 
	}
}
