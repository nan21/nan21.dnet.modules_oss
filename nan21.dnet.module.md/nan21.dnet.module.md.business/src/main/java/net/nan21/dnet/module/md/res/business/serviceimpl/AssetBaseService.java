...
public class AssetBaseService extends AbstractEntityService<AssetBase>
	...
	public List<AssetBase> findByOrgId(Long orgId) {
		return (List<AssetBase>) this.em
			.createQuery("select e from AssetBase e where e.clientId = :pClientId and e.org.id = :pOrgId", AssetBase.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<AssetBase> findByProductId(Long productId) {
		return (List<AssetBase>) this.em
			.createQuery("select e from AssetBase e where e.clientId = :pClientId and e.product.id = :pProductId", AssetBase.class)
			.setParameter("pClientId", Session.user.get().getClientId())
