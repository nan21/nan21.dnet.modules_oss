...
public class CalendarEventService extends AbstractEntityService<CalendarEvent>
	...
	public List<CalendarEvent> findByStatusId(Long statusId) {
		return (List<CalendarEvent>) this.em
			.createQuery("select e from CalendarEvent e where e.clientId = :pClientId and e.status.id = :pStatusId", CalendarEvent.class)
	...
	public List<CalendarEvent> findByPriorityId(Long priorityId) {
		return (List<CalendarEvent>) this.em
			.createQuery("select e from CalendarEvent e where e.clientId = :pClientId and e.priority.id = :pPriorityId", CalendarEvent.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<CalendarEvent> findByBpartnerId(Long bpartnerId) {
		return (List<CalendarEvent>) this.em
			.createQuery("select e from CalendarEvent e where e.clientId = :pClientId and e.bpartner.id = :pBpartnerId", CalendarEvent.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<CalendarEvent> findByContactId(Long contactId) {
		return (List<CalendarEvent>) this.em
			.createQuery("select e from CalendarEvent e where e.clientId = :pClientId and e.contact.id = :pContactId", CalendarEvent.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pContactId", contactId)			 	
			.getResultList(); 
	}
}
