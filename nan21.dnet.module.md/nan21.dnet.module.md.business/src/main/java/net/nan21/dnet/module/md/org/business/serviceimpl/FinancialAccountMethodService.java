...
public class FinancialAccountMethodService extends AbstractEntityService<FinancialAccountMethod>
		implements IFinancialAccountMethodService {
	...
	public FinancialAccountMethod findByAccount_mtd(FinancialAccount financialAccount,PaymentMethod payMethod) {		 
		return (FinancialAccountMethod) this.em
			...
			.setParameter("pPayMethod", payMethod)
			.getSingleResult(); 
	}
	
	public FinancialAccountMethod findByAccount_mtd( Long financialAccountId, Long payMethodId) {
		return (FinancialAccountMethod) this.em
			.createNamedQuery(FinancialAccountMethod.NQ_FIND_BY_ACCOUNT_MTD_PRIMITIVE)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pFinancialAccountId", financialAccountId)
			.setParameter("pPayMethodId", payMethodId)
			.getSingleResult(); 
	}
	
	public List<FinancialAccountMethod> findByFinancialAccount(FinancialAccount financialAccount) {
		return this.findByFinancialAccountId(financialAccount.getId()); 
	}
	
	public List<FinancialAccountMethod> findByFinancialAccountId(Long financialAccountId) {
		return (List<FinancialAccountMethod>) this.em
			.createQuery("select e from FinancialAccountMethod e where e.clientId = :pClientId and e.financialAccount.id = :pFinancialAccountId", FinancialAccountMethod.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pFinancialAccountId", financialAccountId)			 	
			.getResultList(); 
	}
	...
	public List<FinancialAccountMethod> findByPayMethodId(Long payMethodId) {
		return (List<FinancialAccountMethod>) this.em
			.createQuery("select e from FinancialAccountMethod e where e.clientId = :pClientId and e.payMethod.id = :pPayMethodId", FinancialAccountMethod.class)
			.setParameter("pClientId", Session.user.get().getClientId())
			.setParameter("pPayMethodId", payMethodId)			 	
			.getResultList(); 
	}
