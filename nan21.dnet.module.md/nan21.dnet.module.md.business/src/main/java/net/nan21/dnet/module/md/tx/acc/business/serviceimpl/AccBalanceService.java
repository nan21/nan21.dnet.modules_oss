...
public class AccBalanceService extends AbstractEntityService<AccBalance>
	...
	public List<AccBalance> findByOrgId(Long orgId) {
		return (List<AccBalance>) this.em
			.createQuery("select e from AccBalance e where e.clientId = :pClientId and e.org.id = :pOrgId", AccBalance.class)
	...
	public List<AccBalance> findByAccSchemaId(Long accSchemaId) {
		return (List<AccBalance>) this.em
			.createQuery("select e from AccBalance e where e.clientId = :pClientId and e.accSchema.id = :pAccSchemaId", AccBalance.class)
			.setParameter("pClientId", Session.user.get().getClientId())
	...
	public List<AccBalance> findByPeriodId(Long periodId) {
		return (List<AccBalance>) this.em
			.createQuery("select e from AccBalance e where e.clientId = :pClientId and e.period.id = :pPeriodId", AccBalance.class)
			.setParameter("pClientId", Session.user.get().getClientId())
