...
@NamedQueries({
	@NamedQuery(
		name=AccJournal.NQ_FIND_BY_ID,
		query="SELECT e FROM AccJournal e WHERE e.clientId = :pClientId and e.id = :pId ",
		hints=@QueryHint(name=QueryHints.BIND_PARAMETERS, value=HintValues.TRUE)
	)
	,@NamedQuery(
		name=AccJournal.NQ_FIND_BY_IDS,
		query="SELECT e FROM AccJournal e WHERE e.clientId = :pClientId and e.id in :pIds",
		hints=@QueryHint(name=QueryHints.BIND_PARAMETERS, value=HintValues.TRUE)
	)
	,@NamedQuery(
		name=AccJournal.NQ_FIND_BY_NAME,
		query="SELECT e FROM AccJournal e WHERE e.clientId = :pClientId and e.name = :pName",
		hints=@QueryHint(name=QueryHints.BIND_PARAMETERS, value=HintValues.TRUE)
	)
})
@Entity
@Table(
	name=AccJournal.TABLE_NAME
	,uniqueConstraints={
		@UniqueConstraint( 
			name=AccJournal.TABLE_NAME+"_UK1"
