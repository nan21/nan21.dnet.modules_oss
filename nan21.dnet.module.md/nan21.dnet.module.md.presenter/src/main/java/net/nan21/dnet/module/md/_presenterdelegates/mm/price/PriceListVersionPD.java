...
public class PriceListVersionPD extends AbstractPresenterBaseService {

	public void addProductsFromCategory(PriceListVersionDs ds,
			PriceListVersionDsParam params) throws Exception {
		if (params.getProductCategoryId() == null) {
			throw new Exception(
					"No product category given to copy products from!");
		}
		IProductService productService = (IProductService) this
				.findEntityService(Product.class);
		List<Product> products = productService.findByCategoriesId(params
				.getProductCategoryId());

		PriceListVersion priceListVersion = productService.getEntityManager()
				.find(PriceListVersion.class, ds.getId());
		...
		IProductPriceService service = (IProductPriceService) this
				.findEntityService(ProductPrice.class);
		service.insert(result);

	}

	public void addProductsFromPriceListVersion(PriceListVersionDs ds,
			PriceListVersionDsParam params) throws Exception {
		if (params.getCopyFromVersionId() == null) {
			throw new Exception(
					"No source pricelist version given to copy products from!");
		}
		...
		List<Product> products = productService
				...
				.createQuery(
						"select e from "
								...
								+ " e "
								+ " where e.id in (select pp.product.id from "
								+ ProductPrice.class.getSimpleName()
								+ " pp where pp.priceListVersion.id = :sourcePLVersion) ",
						Product.class)
