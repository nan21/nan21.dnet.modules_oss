...
public class CustomerGroupAcctDsConv extends AbstractDsConverter<CustomerGroupAcctDs, CustomerGroupAcct> 
		implements IDsConverter<CustomerGroupAcctDs, CustomerGroupAcct> {
    
    @Override
    protected void modelToEntityReferences(CustomerGroupAcctDs ds, CustomerGroupAcct e, boolean isInsert) throws Exception {
    	if( ds.getCustGroupId() != null  ) {
    		if (e.getCustGroup() == null || !e.getCustGroup().getId().equals(ds.getCustGroupId()) ) {
    			e.setCustGroup( (CustomerGroup) this.em.find(CustomerGroup.class, ds.getCustGroupId() ) );
    		}
    	}
    	else {
    	...
    	if( ds.getSalesAccountId() != null  ) {
    		if (e.getSalesAccount() == null || !e.getSalesAccount().getId().equals(ds.getSalesAccountId()) ) {
    			e.setSalesAccount( (Account) this.em.find(Account.class, ds.getSalesAccountId() ) );
    		}
    	}
    	else {
    ...
    protected void lookup_custGroup_CustomerGroup(CustomerGroupAcctDs ds, CustomerGroupAcct e ) throws Exception {
    	if (ds.getCustGroup() != null && !ds.getCustGroup().equals("") ) {
    		CustomerGroup x = null;
    		try { 
    			x = ((ICustomerGroupService)findEntityService(CustomerGroup.class)).findByCode( ds.getCustGroup() );
    		} catch(javax.persistence.NoResultException exception) {
    			throw new Exception("Invalid value provided to find `CustomerGroup` reference: `custGroup` = " + ds.getCustGroup() + "" );
    		}
    		e.setCustGroup(x); 
    	} else {
    		e.setCustGroup(null);
    ...
    protected void lookup_salesAccount_Account(CustomerGroupAcctDs ds, CustomerGroupAcct e ) throws Exception {
    	if (ds.getSalesAccount() != null && !ds.getSalesAccount().equals("") ) {
    		Account x = null;
    		try { 
    			x = ((IAccountService)findEntityService(Account.class)).findByCode( ds.getSalesAccount() );
    		} catch(javax.persistence.NoResultException exception) {
    			throw new Exception("Invalid value provided to find `Account` reference: `salesAccount` = " + ds.getSalesAccount() + "" );
    		}
    		e.setSalesAccount(x); 
    	} else {
    ...
    protected void lookup_prepayAccount_Account(CustomerGroupAcctDs ds, CustomerGroupAcct e ) throws Exception {
    ...
    protected void lookup_accSchema_AccSchema(CustomerGroupAcctDs ds, CustomerGroupAcct e ) throws Exception {
